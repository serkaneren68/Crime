...
\paragraph{K-Means} (PC1, PC2) düzleminde, sınırları PC1 boyunca kabaca dikey kesimler olan üç küme bulur; bu Bölüm~\ref{sec:pcscatter}'deki gradyanı yansıtır. Gerçek etiketlerle uyumu (ARI $=0{,}14$, NMI $=0{,}15$) mükemmel olmaktan uzak fakat ihmal edilebilir de değildir: etiketlere hiç erişimi olmaksızın PCA düzleminin izotropik bir bölümlemesinin sınıf yapısının önemsiz olmayan bir bölümünü geri kazanması başlı başına, başat sosyoekonomik gradyanın denetim olmaksızın yüzeye çıkacak kadar güçlü olduğunu gösterir. Yine de uyum sınırlıdır; çünkü K-Means PC1 ve PC2'yi izotropik biçimde ele alırken PC2 (varyansın $\%14{,}3$'ü) sınıf-ilgisizdir; dolayısıyla bölümleme çabasının bir kısmı zorunlu olarak toplulukları suçla ilgisiz bir yön boyunca ayırmaya harcanır. PCA düzleminde denetimsiz kümeleme, böylece Bölüm~\ref{sec:pca}--\ref{sec:lda}'da saptanan ayrışmayı miras alır.
...
\begin{table}[!t]
\caption{Düşük ve Yüksek suç sınıfları arasında \texttt{PctKids2Par} üzerinde Welch t-testi sonuçları, etki büyüklüğü ölçüsü olarak Cohen $d$'siyle birlikte (havuzlanmış grup-içi standart sapma).}
\label{tab:ttest}
\centering
\begin{tabular}{lcccc}
\toprule
Sınıf başına $n$ & $\bar x_{\text{Düşük}}$ ($\pm$ s.s.) & $\bar x_{\text{Yüksek}}$ ($\pm$ s.s.) & $t$ \ ($p$-değeri) & Cohen $d$ \\
\midrule
$36$ & $0{,}759\pm 0{,}130$ & $0{,}463\pm 0{,}147$ & $9{,}01$ \ ($2{,}8\!\times\!10^{-13}$) & $2{,}12$ \\
$64$ & $0{,}792\pm 0{,}112$ & $0{,}444\pm 0{,}188$ & $12{,}73$ \ ($7{,}3\!\times\!10^{-23}$) & $2{,}25$ \\
tümü & $-$              & $-$              & $39{,}28$ \ ($\approx 10^{-207}$)    & $2{,}28$ \\
\bottomrule
\end{tabular}
\end{table}

Üç durumun tümünde ($36$, $64$ büyüklüğünde örnekler ve tam sınıflar) $p$-değeri, $\alpha\in\{0{,}05, 0{,}01, 0{,}001\}$ gibi herhangi bir geleneksel anlamlılık düzeyinden onlarca büyüklük mertebesi daha küçüktür; eşit ortalama sıfır hipotezi $H_1$ lehine ezici kanıtla reddedilir. Ancak popülasyona yakın örnek büyüklükleriyle, istatistiksel anlamlılık tek başına zayıf bir özettir; çünkü sıfırdan farklı her ortalama farkı nihayetinde keyfi olarak küçük $p$-değerlerine ulaşır. İçerikli soru, farkın \emph{büyüklüğüdür}. Tablo~\ref{tab:ttest}'in son sütununda raporlanan Cohen $d$'si $n=36$'da yaklaşık $2{,}1$, $n=64$'te $2{,}3$ ve tam sınıflarda $2{,}3$'tür --- bu etki büyüklükleri Cohen'in geleneksel ``çok büyük'' eşiğini ($d\!\geq\!1{,}2$) geniş bir farkla aşar. \texttt{PctKids2Par}'ın Düşük- ve Yüksek-suç dağılımları dolayısıyla yalnızca istatistiksel olarak ayırt edilebilir değil, normalize ölçekte sınıf-içi yayılımın havuzlanmış iki standart sapmasından fazlasıyla birbirinden ayrıktır ($\approx 0{,}79$'a karşı $0{,}45$, $[0,1]$ ölçeğinde $\approx 0{,}33$'lük bir kayma). Beklendiği gibi, örnek büyüklüğünü $36$'dan $64$'e çıkarmak örnek ortalamalarını ve $d$'yi neredeyse değiştirmezken $|t|$'yi $9{,}01$'den $12{,}73$'e yükseltir; bu, $n$ ile birlikte artanın istatistiksel güç olduğunu, altta yatan etki olmadığını gösterir.

\section{SONUÇ}
\label{sec:conclusion}
Bu çalışma, UCI Communities and Crime veri setine eksiksiz bir keşifsel ve istatistiksel analiz boru hattı uygulamıştır. Etiket-kör, hiyerarşik-kümeleme tabanlı bir özellik seçimi $122$ öngörücüyü redundant olmayan $43$ özelliklik bir çalışma kümesine indirgemiş ve sürekli şiddet-suç oranı üç sınıfa ayrıştırılmıştır. Sekiz analizin tümü birlikte okunduğunda, merkezî bulgu tek bir cümlede ifade edilebilir: \emph{bu veri setinde şiddet-suç riski ağırlıklı olarak tek bir sosyoekonomik dezavantaj sürekliliği boyunca düzenlenmiştir.} Pearson ve Spearman korelasyonu, özellik-başına Fisher Uzaklığı, temel-bileşen analizi, doğrusal ayırma analizi ve dört kümeleme yönteminin tümü, çok farklı endüktif eğilimlere rağmen aynı gradyanı dikkate değer bir tutarlılıkla geri kazanır. Bu bölümün geri kalanı, bu tezi analizin daha ayrıntılı bulguları üzerinden detaylandırır.

İlk olarak, aile-yapısı ve sosyoekonomik temsilciler (\texttt{PctKids2Par}, \texttt{PctIlleg}, \texttt{PctPopUnderPov}, \texttt{PctPersDenseHous}, \texttt{PctHousOwnOcc}, \texttt{medIncome}) en güçlü tekil sınıf sinyalini taşır; bu hem Pearson hem Spearman korelasyonu hem de özellik-başına Fisher Uzaklığı tarafından bağımsız olarak doğrulanır (Spearman--Pearson sıra uyumu $0{,}97$); veri-güdümlü küme ayrıca aşırı kalabalıklaşmayı (\texttt{PctPersDenseHous}) elle-seçilmiş bir kümenin gözden kaçırabileceği bir üst korelasyon olarak su yüzüne çıkarır; buna karşılık \texttt{pctUrban}, \texttt{agePct65up} ve \texttt{LandArea} gibi demografik ve coğrafi değişkenler esasen sınıf-bilgisizdir. İkincisi, ayırt edici içerik çok az yönde yoğunlaşmıştır ama artık tek bir başat yönde değil: PC1 varyansın yalnızca $\%18{,}6$'sını açıklar ve $0{,}56$'lık bir Fisher Uzaklığı elde eder --- bu, en iyi iki ham özellikten bile düşüktür --- ikinci en büyük varyans bileşeni PC2 ($\%14{,}3$) ise sınıf-ilgisizdir (Fisher $0{,}06$). Eigenvalue ve Fisher Uzaklığı, dolayısıyla, yalnızca zayıf biçimde ilişkilidir (tamamen PC1 tarafından sürüklenen $r=0{,}76$, $\rho=0{,}49$): yüksek varyans yüksek ayırt ediciliği gerektirmez; bu nokta PC2 tarafından çarpıcı biçimde gösterilir. Üçüncüsü, LDA 2-B gömmesi sınıf ayrılabilirliğini ortaya koymada PCA 2-B gömmesini hem görsel hem nicel olarak açıkça geçer (silüet $0{,}127$'ye karşı $0{,}049$; 5-NN ÇD doğruluğu $\%65{,}9$'a karşı $\%56{,}9$); bunun nedeni tam olarak PCA'nın iki ekseninden birini sınıf-ilgisiz varyansa harcamak zorunda olması, denetimli LDA'nın ise olmamasıdır. Dördüncüsü, denetimsiz kümeleme sınıf yapısının modal değil gradyan-benzeri olduğunu ortaya koyar: PCA düzleminde K-Means yalnızca mütevazı uyum elde eder (ARI $0{,}14$), DBSCAN $233$ gürültü noktasıyla tek baskın kümeye çöker (ARI $0{,}02$) ve hem t-SNE hem SOM, üç sınıfın yumuşakça birbirine geçtiği sürekli bir manifold üretir; yine de SOM, şans düzeyinin çok üstünde, $\%67{,}5$'lik bir ortalama nöron-başına sınıf saflığı elde eder. Son olarak, \texttt{PctKids2Par} üzerinde bir Welch t-testi, eşit sınıf ortalamaları sıfır hipotezini hem $n=36$ ($t=9{,}01$) hem $n=64$ ($t=12{,}73$) için aşırı anlamlılıkla reddeder; daha bilgilendirici biçimde, $\approx 2{,}2$'lik Cohen $d$ etki büyüklüğü Düşük- ve Yüksek-suç dağılımları arasındaki farkı havuzlanmış sınıf-içi standart sapmanın iki katından fazla bir mesafeye yerleştirir; önceki bölümlerin betimsel kanıtını biçimsel olarak doğrular ve \emph{nicelleştirir}.

Genel olarak, veri seti üç iyi-ayrılmış küme olarak değil, şiddet suçunun monotonik biçimde yükseldiği tek-boyutlu bir sosyoekonomik süreklilik olarak en iyi şekilde anlaşılır. Bu geometrinin iki sonucu daha kayda değerdir. İlk olarak, çok-değişkenli Isolation Forest filtresinin özellik-başına IQR kuralıyla karşılaştırılması (Tablo~\ref{tab:outlier}), Yüksek-suç topluluklarının \emph{uç ama anormal olmadığını} gösterir: özellik-başına aykırı değer kuralı bunların yarısından fazlasını atarken birleşik-anomali kuralı yalnızca onda birini atar; bu, bu toplulukların ayrı bir popülasyon oluşturmak yerine var olan bir sürekliliğin uzak ucunda oturduğunu gösterir. İkincisi, gradyanın incelenen her yöntemde tutarlı biçimde başat eksen olması, denetimli bir doğrusal yöntemin (LDA) denetimsiz PCA projeksiyonunu neden geçtiğini, yoğunluk-tabanlı kümelemenin neden başarısız olduğunu ve en güçlü tekil öngörücülerin neden salt demografik değil aile-yapısı, yoksunluk ve konut-kalitesi göstergeleri olduğunu açıklar. Veri-güdümlü $43$-özellik seçimi, hem elle düzenlemenin sübjektifliğini ortadan kaldırır hem de --- PCA varyansı ile sınıf ayrılabilirliği arasındaki ayrışma, aşırı kalabalıklaşmanın yoksulluktan ayrı bir mekanizma olarak rolü ve konut-kalitesi bloğunun kolektif bilgilendiriciliği gibi --- daha küçük, elle-seçilmiş bir çalışma kümesinin gizleyeceği yapıyı açığa çıkarır.

\appendix

% ----------------------------------------------------------------
\section{Tam Pearson Korelasyon Matrisi}
\label{app:fullcorr}
Ana metindeki korelasyon ısı haritası (Şekil~\ref{fig:corrheat}), $44\times 44$'lük bir matrisin sayısal etiketleri sütun genişliğinde okunamayacağı için bu etiketleri atlamaktadır. Hücre düzeyindeki değerleri görmek isteyen okurlar için Şekil~\ref{fig:fullcorr}, aynı matrisi yatay düzende ve tüm girişler görünür biçimde yeniden sunar. Tablo~\ref{tab:features}'in kavramsal-blok ayraçları siyah ızgara çizgileri olarak korunmuş; sağ alt satır ve sütun (ordinal) sınıf korelasyonunu taşımaktadır. Bölüm~\ref{sec:corr}'deki avantaj ve dezavantaj süper-blokları zıt sıcak/soğuk dörtgenler olarak görünür, ve en güçlü tekil sınıf korelasyonları doğrudan en alt satırdan okunabilir.

\begin{sidewaysfigure*}
\centering
\includegraphics[width=0.95\textwidth]{correlation_heatmap_annotated.png}
\caption{$43$ özellik ve ordinal kodlanmış suç sınıfının sayısal etiketlerle tam Pearson korelasyon matrisi. Siyah çizgiler Tablo~\ref{tab:features}'in kavramsal bloklarını ayırır; daha kalın çizgi sınıf satırını ve sütununu izole eder.}
\label{fig:fullcorr}
\end{sidewaysfigure*}

% ----------------------------------------------------------------
\section{Temel Bileşen Yüklemeleri}
\label{app:pcaload}
Bölüm~\ref{sec:pca} temel bileşenleri varyans ve Fisher Uzaklıklarıyla karakterize eder; ancak her bileşenin içerikli (substantive) anlamı orijinal $43$ özellik üzerindeki yüklemelerinde yatar. Tablo~\ref{tab:pcaload}, ilk beş bileşen için en büyük mutlak yüklemeye sahip sekiz özelliği işaretli değerleriyle birlikte raporlar. Üç okuma öne çıkar. PC1 (varyans $\%18{,}6$, Fisher $0{,}56$), Bölüm~\ref{sec:corr}--\ref{sec:pca}'de tanımlanmış ``avantaj--dezavantaj'' eksenidir: yoksulluk ve evlilik-dışı doğum pozitif yüklenirken; gelir, iki-ebeveynli haneler, konut sahipliği ve istihdam negatif yüklenir. PC2 (varyans $\%14{,}3$, Fisher $0{,}06$) bir göç ve etnik-kompozisyon eksenidir (yabancı doğumlu, Asyalı, Hispanik, göçmen sayıları vs ikamet köklülüğü); bu, Bölüm~\ref{sec:pca}'de vurgulanan yüksek-varyanslı, \emph{sınıf-ilgisiz} yöndür. PC3 (varyans $\%8{,}6$) konut-yaşı, toplu taşıma, ikamet istikrarı, yaşlı payı ve yoğunluk özelliklerini toplar; bir kentsel-kırsal / konut-yaşı ekseni olarak yorumlanması en doğrudur ve yine suç gradyanına büyük ölçüde diktir.

\begin{table}[!t]
\caption{Temel bileşen-başına en yüksek yüklemeli özellikler (işaretli yüklemeler, $|\text{yükleme}|$'ye göre sıralanmış). ``Var.'' bileşenin açıklanan-varyans oranı; ``Fisher'' çok-sınıflı Fisher Uzaklığı.}
\label{tab:pcaload}
\centering
\footnotesize
\begin{tabular}{p{0.6cm}p{0.7cm}p{0.7cm}p{5.4cm}}
\toprule
PC & Var. & Fisher & En yüksek yüklemeli özellikler (işaretli) \\
\midrule
...
\addlinespace
PC4 & $0{,}064$ & $0{,}016$ & yüklemeler birçok küçük etkiye yayılmış; tek bir kavramsal blok baskın değil. \\
\addlinespace
PC5 & $0{,}055$ & $0{,}035$ & aynı; kalan varyasyon. \\
\bottomrule
\end{tabular}
\end{table}

% ----------------------------------------------------------------
\section{Özellik Seçiminin Eşik Duyarlılığı}
\label{app:sweep}
Bölüm~\ref{ssec:fselect}'in özellik-seçim hattı, korelasyon-uzaklığı dendrogramının tek bir kesim eşiği $d_{\mathrm{th}}$'ye bağlıdır. Tablo~\ref{tab:sweep}, aynı hiyerarşik-kümeleme prosedürünün $d_{\mathrm{th}}\in[0{,}20, 0{,}60]$ aralığındaki dokuz eşik değerinde döndürdüğü küme sayısını raporlar. Ana analizde kullanılan $d_{\mathrm{th}}=0{,}35$ seçimi, dört kanonik redundancy bloğunun (gelir/varlık, aile yapısı, yoksunluk ve yakın-göç payları) tek gruplara konsolide olduğu, ama kavramsal olarak ayrı boyutların ayrı kaldığı en küçük eşiktir. $d_{\mathrm{th}}$'yi daha da artırmak aile-yapısını ırk/evlilik-dışı doğumla birleştirir (kümeler 6 ve 12); bu noktada veri-güdümlü küme, kayda değer ölçüde daha fazla redundancy çıkarmadan yorumlanabilirliğini kaybeder.

\begin{table}[!t]
\caption{Mutlak Pearson korelasyon uzaklığı üzerinde tam-bağlantı hiyerarşik kümeleme tarafından üretilen küme sayısı, dokuz kesim eşiği için.}
\label{tab:sweep}
\centering
\small
\begin{tabular}{cccc}
\toprule
$d_{\mathrm{th}}$ & küme-içi taban $|r|\geq$ & \#küme & not \\
\midrule
$0{,}20$ & $0{,}80$ & $57$ & yalnızca yakın-yinelemeler birleşir \\
$0{,}25$ & $0{,}75$ & $52$ & \\
$0{,}30$ & $0{,}70$ & $48$ & \\
$0{,}35$ & $0{,}65$ & $43$ & \textbf{bu çalışmada kullanılan çalışma noktası} \\
$0{,}40$ & $0{,}60$ & $41$ & \\
$0{,}45$ & $0{,}55$ & $37$ & \\
$0{,}50$ & $0{,}50$ & $34$ & \\
$0{,}55$ & $0{,}45$ & $31$ & aile $\cup$ ırk kümeleri birleşir \\
$0{,}60$ & $0{,}40$ & $29$ & yorumlanabilirlik önemli ölçüde çöker \\
\bottomrule
\end{tabular}
\end{table}
...
\label{app:repro}
Analiz hattı Python~3.10 ortamında NumPy, pandas, SciPy, scikit-learn~\cite{pedregosa}, seaborn ve MiniSom~\cite{minisom} kullanılarak gerçeklenmiştir. Hat iki aşama hâlinde yapılandırılmıştır: Bölüm~\ref{ssec:fselect}'in hiyerarşik kümelemesini yürüten etiket-kör bir özellik-seçim aşaması ve bu seçimi tüketerek yukarıda raporlanan her şekli ve istatistiği üreten analiz aşaması. Her bildirilen sayının bağımsız olarak doğrulanabilmesi için kaynak kod, üretilen şekiller ve etkileşimli bir Jupyter not defteri ek materyal olarak sunulmuştur. Orijinal veri seti UCI Makine Öğrenmesi Deposundan herkese açık olarak temin edilebilir~\cite{uciccrime}.
